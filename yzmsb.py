...
def main():
    ...
    for file in os.listdir(filedir):
        if fnmatch(file, '*.png'):
            ...
            # 自己的方法
            im = cv2.imread(filedir + '/' + img_name, cv2.IMREAD_GRAYSCALE)
            blur = cv2.GaussianBlur(im, (5, 5), 0)
            ret3, image = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            # ...
            im = naive_remove_noise(image, 4)
            height = len(im)
            width = len(im[0])
            im = im[0:height, int(width / 8):int(width / 8 * 7)]

            # 保存修改后的图片到111文件夹
            cv2.imwrite(f'111/{img_name}', im)
            continue
